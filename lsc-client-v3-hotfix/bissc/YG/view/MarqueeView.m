...
-(instancetype)initWithFrame:(CGRect)frame withTitle:(NSArray *)titles{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        UILabel *tagLabel = [[UILabel alloc]initWithFrame:CGRectMake(30, (self.frame.size.height - 20 )/2, 40, 20)];
        tagLabel.text = @"";
        ...
        self.index = 1;
        self.firstBtn = [self btnframe:CGRectMake(35, 30, [UIScreen mainScreen].bounds.size.width - 2 *50, self.bounds.size.height)  titleColor:_titleColor action:@selector(clickBtn:)];
...
-(void)nextAd{
    ...
    self.moreBtn = [self btnframe: CGRectMake(35, self.bounds.size.height,[UIScreen mainScreen].bounds.size.width -2 *50, self.bounds.size.height)  titleColor:_titleColor action:@selector(clickBtn:)];
