...
#endif
@interface YGForgetViewController (){
    int second;
}
...
@implementation YGForgetViewController
...
-(void) createUI {
  ...
  UIButton *nexBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    ...
    [nexBtn setTitle:@"下一步" forState:(UIControlStateNormal)];
...
//    [nexBtn setBackgroundColor:[UIColor colorWithHexString:@"#1C99FF"]]; #EEEEEE
    [nexBtn setBackgroundColor:[UIColor colorWithHexString:@"#EEEEEE"]]; 
   ...
   UIButton *getVetfiyBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    ...
    [getVetfiyBtn addTarget:self action:@selector(codeAction) forControlEvents:(UIControlEventTouchUpInside)];
    [svContentView addSubview:getVetfiyBtn];
    ...
    [getVetfiyBtn mas_remakeConstraints:^(MASConstraintMaker *make) {
...
-(void)nextAction {
    ...
    YGmodifyPWDViewController *vc = [[YGmodifyPWDViewController alloc] init];
    ...
    }else if (self.companyCodeTF.text.length == 0 && ![self.status isEqualToString:@"1"]){
