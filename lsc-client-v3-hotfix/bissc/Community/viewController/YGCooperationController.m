...
@interface YGCooperationController ()<UITableViewDataSource,UITableViewDelegate,MomentCellDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UITextViewDelegate>
...
@property(nonatomic,strong) NSMutableArray *closeOrOpenFlagArray;
@property(nonatomic,assign) NSInteger seltedSection;
@end

@implementation YGCooperationController
...
- (void)viewDidLoad {
    ...
    //closeOrOpenFlagArray
    self.closeOrOpenFlagArray = [NSMutableArray arrayWithCapacity:3];
    
    for (int i = 0; i <3 ; i ++) {
        [self.closeOrOpenFlagArray addObject:[NSNumber numberWithBool:false]];
    }
     self.seltedSection = 1000;
}
...
-(NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
   //如果是close的话，就是0，open的话，就是cell的个数
    if (![self.closeOrOpenFlagArray[section] boolValue]) {
        return 0;
    }else{
        NSInteger number = 0;
//        cell的个数
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.infoArray.count > 0) {
        ...
        Moment *moment = [self.infoArray objectAtIndex:indexPath.row];
        ...
        cell.textLabel.font = [UIFont systemFontOfSize:15*_Scaling];
        ...
        [cell.imageView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.width.height.mas_equalTo(40*_Scaling);
        ...
        //管理员的话，加个icon    UIButton *adminIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"组 38 拷贝"]];
        UIButton *adminIcon = [UIButton buttonWithType:UIButtonTypeCustom];

        [adminIcon setTitle:@"管理员" forState:UIControlStateNormal];
        adminIcon.titleLabel.font = [UIFont systemFontOfSize:11*_Scaling];
        [adminIcon setTitleColor:[UIColor colorWithHexString:@"#007AFF"] forState:UIControlStateNormal];
        adminIcon.layer.borderColor =[UIColor colorWithHexString:@"#007AFF"].CGColor;
        adminIcon.layer.borderWidth=0.5;
        
        [cell.contentView addSubview:adminIcon];
        adminIcon.hidden = !moment.isAdmin;
        
        
        [adminIcon mas_makeConstraints:^(MASConstraintMaker *make) {
            make.leading.equalTo(cell.textLabel.mas_trailing).offset(10);
//            make.left.equalTo(cell.textLabel.right).offset(10);
            make.centerY.mas_equalTo(cell.textLabel);
             make.width.mas_equalTo(40*_Scaling);
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Moment *model = self.infoArray[indexPath.row];
...
-(UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    [lineView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(5*_Scaling);
        make.top.mas_equalTo(0);
        
    }];
    UILabel *lb= [[UILabel alloc] init];
    lb.font = [UIFont systemFontOfSize:16*_Scaling];
    lb.textColor =[UIColor blackColor];
    [view addSubview:lb];
    [lb mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(18*_Scaling);
        make.height.mas_equalTo(18*_Scaling);
        make.top.mas_equalTo(23*_Scaling);
...
//    icon.image = [UIImage imageNamed:@"chat 图层 7"];
    [icon setImage:[UIImage imageNamed:@"chat 图层 8"] forState:UIControlStateNormal];
    [icon setImage:[UIImage imageNamed:@"chat 图层 7"] forState:UIControlStateSelected];
    [view addSubview:icon];
    [icon mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(78*_Scaling);
        make.height.mas_equalTo(20*_Scaling);
        make.top.mas_equalTo(21*_Scaling);
    ...
    UILabel *RightLbt= [[UILabel alloc] init];
    RightLbt.font = [UIFont systemFontOfSize:16*_Scaling];
    RightLbt.textColor =[UIColor colorWithHexString:@"#CCCCCC"];
    [view addSubview:RightLbt];
    [RightLbt mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(-18*_Scaling);
        make.height.mas_equalTo(15*_Scaling);
        make.top.mas_equalTo(23*_Scaling);
    ...
    if (section ==0) {
      ...
      icon.selected = self.seltedSection ==section;
    view.userInteractionEnabled = YES;
    UITapGestureRecognizer   *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(openOrCloseCell:)];
...
-(void)openOrCloseCell:(UITapGestureRecognizer *)tap{
    
     NSInteger index = tap.view.tag-1000 ;
    
    if ([self.closeOrOpenFlagArray[index] boolValue]) {
        [self.closeOrOpenFlagArray replaceObjectAtIndex:index withObject:[NSNumber numberWithBool:false]];
        self.seltedSection = 1000;
    }else {
        [self.closeOrOpenFlagArray removeAllObjects];
        for (int i = 0; i < 3; i ++) {
            [self.closeOrOpenFlagArray addObject:[NSNumber numberWithBool:false]];
        }
        [self.closeOrOpenFlagArray replaceObjectAtIndex:index withObject:[NSNumber numberWithBool:true]];
        self.seltedSection = index;
    }
...
//    for (int i = 0; i<3 ; i ++) {
//        if(tag == i){
//            [self.closeOrOpenFlagArray addObject:[NSNumber numberWithBool:true]];
//        }else{
//             [self.closeOrOpenFlagArray addObject:[NSNumber numberWithBool:false]];
//        }
//    }
...
//
//
//    NSLog(@"openOrCloseCell,%ld",tap.view.tag);
}
